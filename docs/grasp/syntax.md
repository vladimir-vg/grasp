...
key_type       ::= "boolean" | "i64" | "f64" | "string" | "bytes"
                 | "date" | "time" | "timestamp" | "interval"
                 | type_var                    -- only in a variant
type_var       ::= [A-Z][a-zA-Z0-9]*
```

`key_type` is the scalars, because a dict is an object on the wire and an
object's keys are strings — so a key type needs one string spelling its own type
reads back. A type variable is admitted there because it stands for a type the
typespec has not named: `dict(K, V)` is how the library writes `dict:get`.
...
| ``cannot be a dict key: a key is a scalar`` | `dict(K,V)` with a `K` that has no string spelling |
