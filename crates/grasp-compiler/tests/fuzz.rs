...
//! it *comes back*.
//!
//! The first run of this found an unbound variable inside a call argument
//! reaching the planner, which a release build then emitted as a program
//! reading a column that did not exist.
...
const TOKENS: &[&str] = &[
    "(", ")", "[", "]", "{", "}", ":", ",", ".", "_", "*", "**", ":=", "::", "<-", "->", "=>",
    "not", "and", "or", "input", "true", "false", "NONE", "null", "\n", "    ", " ", "\"", "\\",
    "-", "+", "/", "%", "++", "=", "==", "!=", "<", "<=", ">", ">=", "#", "0", "1.5",
    "9999999999999999999999", "record", "array", "dict", "optional", "json", "dynamic", "bytes",
    "date", "time", "timestamp", "interval", "sum", "count", "min", "max", "avg", "function",
    "circuit", "fixpoint", "empty()", "constant([])", "map", "join", "filter", "cast",
    "coalesce", "if", "get", "slice", "map_array", "filter_array", "string:length", "array:at",
    "dict:get", "temporal:date", "dynamic:of", "\u{e9}", "\u{0}", "\t",
];
