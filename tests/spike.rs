...
fn row(id: i64, dept: i64, salary: i64) -> DynValue {
    DynValue::record([DynValue::I64(id), DynValue::I64(dept), DynValue::I64(salary)])
}
...
fn typed_api_instantiates_at_one_value_type() {
    let (mut dbsp, (emp_in, dept_in, joined_out, agg_out)) = Runtime::init_circuit(1, |circuit| {
        ...
        let emp_idx = emp.map_index(|r: &DynValue| {
            (r.field(1).cloned().unwrap_or(DynValue::None), r.clone())
        });
        // dept rows are (id, name); key on field 0.
        let dept_idx = dept.map_index(|r: &DynValue| {
            (r.field(0).cloned().unwrap_or(DynValue::None), r.clone())
        });
    ...
    dept_in.push(DynValue::record([DynValue::I64(10), DynValue::str("eng")]), 1);
    dept_in.push(DynValue::record([DynValue::I64(20), DynValue::str("ops")]), 1);
